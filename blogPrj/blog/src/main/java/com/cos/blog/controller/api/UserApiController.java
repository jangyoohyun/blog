...
public class UserApiController {
	...
	@PutMapping("/user")
	public ResponseDto<Integer> update(@RequestBody User user) { // key=value, x-www-form-encoded
		userService.회원수정(user);
		// 여기서는 트랜잭션이 종료되기 때문에 DB에 값은 변경이 됐지만
		// 세션 값은 변경되지 않은 상태이기 때문에 직접 세션 값을 변경해줘야 함.
		// 세션 등록
	 ...
	 * userService.로그인(user); // principal (접근 주체) if(principal != null) {
