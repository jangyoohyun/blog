...
public class PrincipalDetail implements UserDetails{
	...
	// 계정이 잠겨있는지 리턴한다. (true: 잠겨있지 않음)
	@Override
	public boolean isAccountNonLocked() {
		return true;
	}

	// 비밀번호가 만료되지 않았는지 리턴한다. (true: 비밀번호가 만료되지 않음)
	@Override
	...
	// 계정이 갖고있는 권한 목록을 리턴한다. (권한이 여러 개 있을 수 있어서 루프를 돌아야 하는데 우린 한개)
	@Override
	public Collection<? extends GrantedAuthority> getAuthorities() {
		
		Collection<GrantedAuthority> collecters = new ArrayList<>();
		collecters.add(()->{ return "ROLE_"+user.getRole();});
